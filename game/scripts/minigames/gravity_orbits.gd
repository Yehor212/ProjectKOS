...
## Vatavu 2015 IJHCS: toddler (2-4) needs 15-20mm targets = 120-165px at 7" 720p
## Nacher 2015 IJHCS: preschool (4-7) needs 12-15mm = 100-124px
const STAR_TOUCH_RADIUS_TODDLER: float = 75.0   ## 150px diameter ≈ 18mm
const STAR_TOUCH_RADIUS_PRESCHOOL: float = 60.0  ## 120px diameter ≈ 14.5mm
...
var _star_touch_r: float = 60.0  ## Обчислюється в _start_game()
...
func _ready() -> void:
	...
	_star_touch_r = STAR_TOUCH_RADIUS_TODDLER if _is_toddler else STAR_TOUCH_RADIUS_PRESCHOOL
	super()
...
func _start_round() -> void:
	...
	if not _is_toddler:
		...
		for i: int in asteroid_count:
			_spawn_asteroid(vp)
		## L8: R1 asteroid tutorial — brief "X" wobble on first asteroid to teach avoidance
		if r == 0 and _all_round_nodes.size() > 0:
			for node: Node in _all_round_nodes:
				if is_instance_valid(node) and node.has_meta("is_asteroid") and node.get_meta("is_asteroid"):
					_animate_asteroid_warning(node)
					break
...
func _start_breath_animation() -> void:
	...
	if _breath_tween and _breath_tween.is_valid():
		_breath_tween.kill()
	_breath_tween = _create_game_tween()
...
func _spawn_catch_zone_ring() -> void:
	...
	var ring_radius: float = _planet_radius + _star_touch_r
	...
	if not SettingsManager.reduced_motion:
		...
		_catch_zone_tween = _create_game_tween()
...
func _spawn_star(vp: Vector2) -> void:
	...
	## Розмір: Toddler = 120dp (Hourcade 2007: 20mm), Preschool = 80dp, Gold = 90dp
	var star_sz: float
	if _is_toddler:
		star_sz = 120.0
...
func _spawn_asteroid(vp: Vector2) -> void:
	...
	asteroid_node.set_meta("is_asteroid", true)
...
func _random_entry_position_and_velocity(vp: Vector2, item_size: float,
	...
	## L3: slow_mode — reduce speed by 50% for motor-impaired children
	if SettingsManager.slow_mode:
		speed *= 0.5
...
func _input(event: InputEvent) -> void:
	...
	for item: Dictionary in _floating_items:
		...
		var touch_r: float = maxf(_star_touch_r, item_size * 0.5)
...
func _catch_star(item: Dictionary) -> void:
	...
	## 1. Заморозити + pop
	var freeze_tw: Tween = _create_game_tween()
	...
	if _planet_node and is_instance_valid(_planet_node) and not SettingsManager.reduced_motion:
		...
		eat_tw.tween_callback(func() -> void:
			if is_instance_valid(self):
				...
				var reset_tw: Tween = _create_game_tween()
...
func _handle_asteroid_tap(item: Dictionary) -> void:
	...
	## Планета тремтить
	if _planet_node and is_instance_valid(_planet_node) and not SettingsManager.reduced_motion:
		var scared_tw: Tween = _create_game_tween()
...
## L8: Коротка анімація попередження на першому астероїді R1 — навчити уникати
func _animate_asteroid_warning(node: Node2D) -> void:
	if not is_instance_valid(node) or SettingsManager.reduced_motion:
		return
	var tw: Tween = _create_game_tween()
	## Тремтіння + тимчасовий "X" overlay
	tw.tween_property(node, "rotation", deg_to_rad(15.0), 0.1)
	tw.tween_property(node, "rotation", deg_to_rad(-15.0), 0.1)
	tw.tween_property(node, "rotation", 0.0, 0.1)
	tw.tween_property(node, "modulate", Color(1.0, 0.3, 0.3, 1.0), 0.15)
...
func _finish() -> void:
	...
	if _planet_node and is_instance_valid(_planet_node):
		...
		if not SettingsManager.reduced_motion:
			var final_tw: Tween = _create_game_tween()
