...
const TODDLER_SCALE: float = 1.5  ## Vatavu 2015: 120px+ for ages 2-4 (was 1.4)
const TODDLER_SNAP_RADIUS: float = 150.0  ## NNGroup: 2-3yo need 2cm+ touch targets
...
## Cross-session adaptive difficulty (Wilson 2019: 85% accuracy rule)
## _cross_session_offset: -1 (struggling), 0 (on-track), +1 (advanced)
## _cross_session_factor: 0.0 (easier) .. 1.0 (harder) from MasteryManager history
var _cross_session_offset: int = 0
var _cross_session_factor: float = 0.5
...
## Background interactions — Stahl & Feigenson 2015: surprise enhances learning
var _bg_interactions: Array[Dictionary] = []
var _bg_interact_cooldown: bool = false
...
func _ready() -> void:
	...
	## Cross-session DDA: load historical difficulty from MasteryManager
	_init_cross_session_difficulty()
...
func finish_game(earned_stars: int, stats: Dictionary = {}) -> void:
	...
	if earned_stars > 0 and _ui_layer \
		...
		## Rare super-celebration (1/8 = 12.5%) — variable ratio reinforcement
		## Stahl & Feigenson 2015: surprise enhances learning retention
		## Only on 3+ stars: reward good performance with delight
		if earned_stars >= 3 and randf() < 0.125:
			_trigger_rare_celebration(vp)
...
func _apply_background() -> void:
	...
	_setup_bg_interactions(vp_size)
...
## Rare super-celebration: confetti rain + rainbow + gold burst + bonus SFX.
## Deci & Ryan SDT: autonomy-compatible surprise boosts intrinsic motivation.
func _trigger_rare_celebration(vp: Vector2) -> void:
	## Multi-wave confetti rain (3 waves, staggered)
	VFXManager.spawn_premium_confetti_rain(vp)
	...
	get_tree().create_timer(0.6).timeout.connect(func() -> void:
		if is_instance_valid(self):
			VFXManager.spawn_premium_confetti_rain(vp))
	## Rainbow ring from center
	VFXManager.spawn_rainbow_ring(vp / 2.0)
	## Golden bursts from corners (firework pattern)
	VFXManager.spawn_golden_burst(Vector2(vp.x * 0.2, vp.y * 0.3))
	VFXManager.spawn_golden_burst(Vector2(vp.x * 0.8, vp.y * 0.3))
	## Bonus SFX — celebration fanfare
	...
	## Extra screen shake (gentle — child-safe < 3Hz)
...
func _register_error(node: Node2D = null) -> void:
	...
	## M8: Scaffolding fading — experienced players get slightly more attempts before hint
	## Research: gradual scaffold removal produces better outcomes (UCLan thesis, Structural Learning)
	var base_threshold: int = 2 if SettingsManager.age_group == 1 else 3
	var mastery_bonus: int = 1 if ProgressManager.games_played >= 15 else 0
	var threshold: int = base_threshold + mastery_bonus
...
## Ініціалізувати крос-сесійну складність з MasteryManager.
## Wilson et al. 2019 (Nature Communications): 85% accuracy = optimal learning.
## Дитина що повертається повинна продовжити з відповідного рівня, не з нуля.
func _init_cross_session_difficulty() -> void:
	if game_id.is_empty():
		return  ## game_id ще не задано — дочірній клас задасть у своєму _ready()
	_cross_session_offset = MasteryManager.get_difficulty_offset(game_id)
	## Конвертуємо offset (-1,0,+1) у factor (0.0..1.0) для плавного блендінгу
	match _cross_session_offset:
		-1:
			_cross_session_factor = 0.2  ## Struggling → починай легше
		0:
			_cross_session_factor = 0.5  ## On-track → стандартна прогресія
		1:
			_cross_session_factor = 0.8  ## Advanced → починай складніше
		_:
			_cross_session_factor = 0.5
...
func _get_adaptive_factor() -> float:
	if _round_errors.size() < 2:
		return _cross_session_factor  ## Перші раунди — використати крос-сесійні дані
...
## Адаптивна інтерполяція — 50% лінійна + 30% within-session + 20% cross-session.
## Wilson 2019: maintain ~85% accuracy for optimal learning zone.
func _scale_adaptive(easy_val: float, hard_val: float, current: int, total: int) -> float:
	...
	var session_factor: float = _get_adaptive_factor()
	var session_target: float = lerpf(easy_val, hard_val, session_factor)
	var cross_target: float = lerpf(easy_val, hard_val, _cross_session_factor)
	## Blend: 50% linear progression + 30% within-session + 20% cross-session history
	return linear * 0.5 + session_target * 0.3 + cross_target * 0.2
...
## ========== INTERACTIVE BACKGROUND — Stahl & Feigenson 2015 ==========
## Tap cloud → rain, tap flower → butterfly, tap sun → glow, etc.
## Purely decorative — no gameplay impact. 3s cooldown prevents spam.

func _setup_bg_interactions(vp: Vector2) -> void:
	_bg_interactions.clear()
	match bg_theme:
		"meadow", "default", "garden":
			_bg_interactions = [
				{"pos": Vector2(vp.x * 0.85, vp.y * 0.06), "r": 55.0, "type": "cloud"},
				{"pos": Vector2(vp.x * 0.12, vp.y * 0.82), "r": 45.0, "type": "flower"},
			]
		"forest":
			_bg_interactions = [
				{"pos": Vector2(vp.x * 0.88, vp.y * 0.18), "r": 55.0, "type": "tree"},
				{"pos": Vector2(vp.x * 0.08, vp.y * 0.72), "r": 40.0, "type": "mushroom"},
			]
		"ocean":
			_bg_interactions = [
				{"pos": Vector2(vp.x * 0.88, vp.y * 0.05), "r": 50.0, "type": "sun"},
				{"pos": Vector2(vp.x * 0.5, vp.y * 0.88), "r": 55.0, "type": "wave"},
			]
		"sky":
			_bg_interactions = [
				{"pos": Vector2(vp.x * 0.2, vp.y * 0.06), "r": 50.0, "type": "cloud"},
				{"pos": Vector2(vp.x * 0.78, vp.y * 0.04), "r": 45.0, "type": "sun"},
			]
		"space":
			_bg_interactions = [
				{"pos": Vector2(vp.x * 0.92, vp.y * 0.12), "r": 35.0, "type": "twinkle"},
				{"pos": Vector2(vp.x * 0.12, vp.y * 0.18), "r": 35.0, "type": "twinkle"},
			]
		"candy":
			_bg_interactions = [
				{"pos": Vector2(vp.x * 0.1, vp.y * 0.1), "r": 45.0, "type": "sparkle"},
				{"pos": Vector2(vp.x * 0.88, vp.y * 0.85), "r": 45.0, "type": "sparkle"},
			]
		"city":
			_bg_interactions = [
				{"pos": Vector2(vp.x * 0.82, vp.y * 0.05), "r": 50.0, "type": "cloud"},
				{"pos": Vector2(vp.x * 0.1, vp.y * 0.75), "r": 40.0, "type": "flower"},
			]
		"science":
			_bg_interactions = [
				{"pos": Vector2(vp.x * 0.9, vp.y * 0.12), "r": 40.0, "type": "twinkle"},
			]
		"puzzle":
			_bg_interactions = [
				{"pos": Vector2(vp.x * 0.88, vp.y * 0.06), "r": 50.0, "type": "cloud"},
			]


func _check_bg_interaction(event: InputEvent) -> void:
	if _game_finished or _bg_interact_cooldown or _bg_interactions.is_empty():
		return
	...
	if not tapped:
		return
	for zone: Dictionary in _bg_interactions:
		var zone_pos: Vector2 = zone.get("pos", Vector2.ZERO) as Vector2
		var zone_r: float = zone.get("r", 40.0) as float
		if pos.distance_to(zone_pos) <= zone_r:
			_fire_bg_effect(zone_pos, zone.get("type", "") as String)
			return


func _fire_bg_effect(pos: Vector2, type: String) -> void:
	_bg_interact_cooldown = true
	AudioManager.play_sfx("click", randf_range(1.3, 1.6))
	match type:
		"cloud":
			_bg_fx_rain(pos)
		"flower":
			_bg_fx_butterfly(pos)
		"tree":
			_bg_fx_bird(pos)
		"mushroom", "sparkle":
			_bg_fx_sparkle(pos)
		...
		"twinkle":
			_bg_fx_twinkle(pos)
	## 3s cooldown (child-safe: prevents overstimulation)
	var timer: SceneTreeTimer = get_tree().create_timer(3.0)
	timer.timeout.connect(func() -> void:
		if is_instance_valid(self):
			_bg_interact_cooldown = false)


func _bg_fx_rain(pos: Vector2) -> void:
	for i: int in 5:
		var drop: Node2D = Node2D.new()
		drop.position = pos + Vector2(randf_range(-25, 25), 0)
		drop.z_index = -1
		drop.draw.connect(func() -> void:
			drop.draw_rect(Rect2(-1.0, -4.0, 2.0, 8.0), Color("64b5f6", 0.7)))
		add_child(drop)
		var tw: Tween = _create_game_tween()
		var delay: float = float(i) * 0.08
		tw.tween_property(drop, "position:y", pos.y + 100.0,
			randf_range(0.4, 0.7)).set_delay(delay)
		tw.parallel().tween_property(drop, "modulate:a", 0.0, 0.25).set_delay(delay + 0.35)
		tw.tween_callback(drop.queue_free)


func _bg_fx_butterfly(pos: Vector2) -> void:
	var b: Node2D = Node2D.new()
	b.position = pos
	b.z_index = -1
	var wc: Color = [Color("ff6f91"), Color("ffc75f"), Color("a78bfa")].pick_random()
	b.draw.connect(func() -> void:
		b.draw_circle(Vector2(-5, 0), 4.5, wc)
		b.draw_circle(Vector2(5, 0), 4.5, wc)
		b.draw_circle(Vector2(-5, -4), 3.5, wc.lightened(0.2))
		b.draw_circle(Vector2(5, -4), 3.5, wc.lightened(0.2))
		b.draw_rect(Rect2(-0.5, -3, 1, 7), Color(0.3, 0.2, 0.1)))
	add_child(b)
	var target: Vector2 = pos + Vector2(randf_range(-80, 80), randf_range(-60, -120))
	var tw: Tween = _create_game_tween()
	tw.tween_property(b, "position", target, 1.2).set_trans(Tween.TRANS_SINE)
	tw.parallel().tween_property(b, "modulate:a", 0.0, 0.4).set_delay(0.8)
	tw.tween_callback(b.queue_free)


func _bg_fx_bird(pos: Vector2) -> void:
	var bird: Node2D = Node2D.new()
	bird.position = pos
	bird.z_index = -1
	bird.draw.connect(func() -> void:
		bird.draw_arc(Vector2(-5, 0), 5.0, PI * 0.2, PI * 0.8, 8,
			Color(0.3, 0.3, 0.3), 2.0, true)
		bird.draw_arc(Vector2(5, 0), 5.0, PI * 0.2, PI * 0.8, 8,
			Color(0.3, 0.3, 0.3), 2.0, true))
	add_child(bird)
	var tw: Tween = _create_game_tween()
	tw.tween_property(bird, "position", pos + Vector2(180, -90), 1.0) \
		.set_trans(Tween.TRANS_QUAD).set_ease(Tween.EASE_IN)
	tw.parallel().tween_property(bird, "modulate:a", 0.0, 0.3).set_delay(0.7)
	tw.tween_callback(bird.queue_free)


func _bg_fx_sparkle(pos: Vector2) -> void:
	var colors: Array[Color] = [Color("ffd700"), Color("ff69b4"), Color("00e5ff")]
	for i: int in 4:
		var spark: Node2D = Node2D.new()
		spark.position = pos
		spark.z_index = -1
		var sc: Color = colors[i % colors.size()]
		spark.draw.connect(func() -> void:
			spark.draw_circle(Vector2.ZERO, 3.0, sc))
		add_child(spark)
		var angle: float = float(i) / 4.0 * TAU + randf_range(-0.3, 0.3)
		var target: Vector2 = pos + Vector2(cos(angle), sin(angle)) * randf_range(20, 45)
		var tw: Tween = _create_game_tween()
		tw.tween_property(spark, "position", target, 0.5) \
			.set_trans(Tween.TRANS_QUAD).set_ease(Tween.EASE_OUT)
		tw.parallel().tween_property(spark, "modulate:a", 0.0, 0.25).set_delay(0.25)
		tw.tween_callback(spark.queue_free)


func _bg_fx_sunbeam(pos: Vector2) -> void:
	var beam: Node2D = Node2D.new()
	beam.position = pos
	beam.z_index = -1
	beam.modulate = Color(1, 1, 1, 0)
	beam.draw.connect(func() -> void:
		beam.draw_circle(Vector2.ZERO, 30.0, Color("fff176", 0.3))
		beam.draw_circle(Vector2.ZERO, 16.0, Color("fff9c4", 0.5)))
	add_child(beam)
	var tw: Tween = _create_game_tween()
	tw.tween_property(beam, "modulate:a", 1.0, 0.25)
	tw.tween_property(beam, "scale", Vector2(1.5, 1.5), 0.4).set_trans(Tween.TRANS_SINE)
	tw.parallel().tween_property(beam, "modulate:a", 0.0, 0.4)
	tw.tween_callback(beam.queue_free)


func _bg_fx_splash(pos: Vector2) -> void:
	for i: int in 3:
		var drop: Node2D = Node2D.new()
		drop.position = pos
		drop.z_index = -1
		drop.draw.connect(func() -> void:
			drop.draw_circle(Vector2.ZERO, 4.0, Color("64b5f6", 0.6)))
		add_child(drop)
		var angle: float = PI + randf_range(-0.5, 0.5)
		var target: Vector2 = pos + Vector2(cos(angle), sin(angle)) * randf_range(25, 50)
		var tw: Tween = _create_game_tween()
		tw.tween_property(drop, "position", target, 0.45) \
			.set_trans(Tween.TRANS_QUAD).set_ease(Tween.EASE_OUT)
		tw.parallel().tween_property(drop, "modulate:a", 0.0, 0.2).set_delay(0.25)
		tw.tween_callback(drop.queue_free)


func _bg_fx_twinkle(pos: Vector2) -> void:
	var star: Node2D = Node2D.new()
	star.position = pos
	star.z_index = -1
	star.modulate = Color(1, 1, 1, 0)
	star.draw.connect(func() -> void:
		star.draw_circle(Vector2.ZERO, 6.0, Color("fff9c4", 0.8))
		star.draw_rect(Rect2(-1, -10, 2, 20), Color("ffffff", 0.6))
		star.draw_rect(Rect2(-10, -1, 20, 2), Color("ffffff", 0.6)))
	add_child(star)
	var tw: Tween = _create_game_tween()
	tw.tween_property(star, "modulate:a", 1.0, 0.12)
	tw.tween_property(star, "scale", Vector2(1.6, 1.6), 0.25)
	tw.parallel().tween_property(star, "modulate:a", 0.0, 0.25)
