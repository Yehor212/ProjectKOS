...
## Toddler: перетягування годинної стрілки (фізична маніпуляція), 3 раунди (:00).
...
func get_tutorial_demo() -> Dictionary:
	if not _clock_face or not is_instance_valid(_clock_face):
		return {}
	if _is_toddler:
		## Drag демо: від центру до цільової позиції стрілки
		var angle: float = deg_to_rad(float(_target_hour % 12) * 30.0 - 90.0)
		var target_pos: Vector2 = _clock_center_global + Vector2(
			cos(angle) * _active_clock_radius * 0.6,
			sin(angle) * _active_clock_radius * 0.6)
		return {"type": "drag", "from": _clock_center_global, "to": target_pos}
...
func _unhandled_input(event: InputEvent) -> void:
	super(event)
	if not _is_toddler or _input_locked or _game_over:
		return  ## Не обробляємо drag поза Toddler або при блокуванні
	## Mouse (desktop) та touch (Android) — через emulate_touch_from_mouse
...
func _show_idle_hint() -> void:
	...
	if level >= 2:
		...
		_reset_idle_timer()
		return
	## Пульсуємо годинник (A10 level 0-1)
	if is_instance_valid(_clock_face):
